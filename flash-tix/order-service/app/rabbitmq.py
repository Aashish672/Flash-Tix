...
def publish_order_cancelled(order_id, event_id, quantity):
    retries = 20
    while retries > 0:
        try:
            connection=pika.BlockingConnection(pika.ConnectionParameters(host='rabbitmq', heartbeat=600))
            ...
            connection.close()
            print(f"Successfully published order_cancelled for order {order_id}")
            return
        except (pika.exceptions.AMQPConnectionError, pika.exceptions.ConnectionClosedByBroker):
            retries -= 1
            time.sleep(5)
        except Exception as e:
            print(f"Unexpected error in publish_order_cancelled: {e}")
            break